...
function GitHub.InstallRepo(link, dir, parent, routines, specs)
	...
	if scriptCount > 0 then
		...
		local function LocateFolder(folderName)
			...
			if count > dir then
				...
				if (parent and parent.Name) ~= folderName and "Modules" ~= folderName then
					local Generated
					parent, Generated = GitHub.GetFirstChild(
						parent,
						folderName,
						"Folder"
					)
		...
		coroutine.resume(coroutine.create(GitHub.InstallScript), "https://raw.githubusercontent.com" .. scriptLink:gsub("(/[^/]+/[^/]+/)blob/", "%1", 1), scriptAsset)
		...
		for a = 2, scriptCount do
			...
			coroutine.resume(coroutine.create(GitHub.InstallScript), "https://raw.githubusercontent.com" .. link:gsub("(/[^/]+/[^/]+/)blob/", "%1", 1), scriptAsset)
		end
	end
	
	for index = 1, folderCount do
		local link = folders[index]
		coroutine.resume(coroutine.create(GitHub.InstallRepo), "https://github.com" .. link, dir, parent, routines, specs)
	end
...
function GitHub:Download(url, parent, list)
	...
	if name and (name == "_" or name:lower() == "main" or name:lower() == "init") then
		return GitHub:Download("https://github.com/" .. org .. "/" .. repo .. "/tree/" .. (tree or "master") .. "/" .. dir:gsub("/[^/]+$", ""):gsub("^/", ""), parent, list)
	end
	...
	elseif org then
		...
		local object = GitHub.GetFirstChild(
			parent,
			org,
			"Folder"
		)
